...
double* GD::plainlinHinge(long approxDeg, double** zData, double* ip, long factorDim, long sampleDim, double gamma) {
	double* grad = new double[factorDim]();
	
	for (long i = 0; i < factorDim; ++i) {
		for (long j = 0; j < sampleDim; ++j) {
			grad[i] += (degree4[1] + (pow(ip[j], 2) - pow(ip[j]-1, 2)) * degree4[2] + (pow(ip[j], 4) -pow(ip[j]-1, 4) )* degree4[3]) * zData[j][i];
//			grad[i] += (degree4[0] + (ip[j]-1) * degree4[1]  + pow(ip[j]-1, 2) * degree4[2] + pow(ip[j]-1, 4) * degree4[3]) * zData[j][i];
...
//	cout << "plain ip = " <<  *ip << endl;
	//sigmoid
	double* grad = plainSigmoid(approxDeg, zData, ip, factorDim, sampleDim, gamma);
	//linearized hinge
//	double* grad = plainlinHinge(approxDeg, zData, ip, factorDim, sampleDim, gamma);
...
void GD::trueNLGDiteration(double** zData, double* wData, double* vData, long factorDim, long sampleDim, double gamma, double eta) {
	...
	double ip;
	for(long j = 0; j < sampleDim; ++j) {
		ip = trueIP(vData, zData[j], factorDim);
		//sigmoid
		double tmp = (ip > 15.0) ? 0 : (ip < -15.0) ? -1.0 : - 1. / (1. + exp(ip));
		//linearized hinge
//		double tmp = (ip < 0) ? 1 : (ip > 1) ? 0 : 1 - ip;
//		double tmp = (ip < 1) ? 1 - ip : 0;
